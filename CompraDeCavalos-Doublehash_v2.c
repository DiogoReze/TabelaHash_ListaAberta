...
int funcDisperDupla(int chave){
    //https://www.geeksforgeeks.org/double-hashing/
    int menorPrimo = 13;
    int base1 = funcEspalhamento(chave);
    int base2 = menorPrimo - (chave % menorPrimo);
    int pos;// = (base1 + i * base2) % M ;
    int i;

    //tratamento de colisoes
    for (i = 0; i < M; i++){
        pos = (base1 + i * base2) % M ;
        if (tabelaHash[pos].id == tempTabelaHash[pos].id){
            return pos;
...
void inserirCavalo(){
  ...
  int indice = funcDisperDupla(cavalo.id);
