...
void inserirCavalo(){
  ...
  while(tabelaHash[indice].id != 0){
    indice = funcEspalhamento(funcEspalhamento(cavalo.id) + c1 * contador + c2 * contador * contador) % M;
...
CompraCavalos* buscarCavalo(int chave){
  ...
  while(tabelaHash[indice].id != 0){
    if(tabelaHash[indice].id == chave){
	  ...
	  indice = funcEspalhamento(funcEspalhamento(chave) + c1 * contador + c2 * contador * contador) % M;
      //indice = funcEspalhamento(funcEspalhamento(chave) + pow(contador, 2)) % M;
  	  contador++;
    }
  }
